//
//  Fonts.swift
//  UMCignal
//
//  Created by 이승준 on 4/5/25.
//

import UIKit

struct Fonts {
    static let H1 = UIFont(name: "Pretendard-Bold", size: 22)
    static let T1 = UIFont(name: "Pretendard-SemiBold", size: 20)
    static let T2 = UIFont(name: "Pretendard-Bold", size: 18)
    static let B1 = UIFont(name: "Pretendard-SemiBold", size: 16)
    static let B2 = UIFont(name: "Pretendard-Bold", size: 14)
    static let B3 = UIFont(name: "Pretendard-Bold", size: 12)
    static let Detail = UIFont(name: "Pretendard-Medium", size: 10)
}
